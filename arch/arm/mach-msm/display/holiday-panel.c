...
static struct mdp_reg holiday_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
#if 0
static struct mdp_reg holiday_auo_gamma[] = {
...
static struct mdp_reg holiday_sharp_gamma[] = {
	{0x94800, 0x000000, 0x0},
...
/* TODO: acquire gamma finetune data from Logic */
#if 0
...
static struct gamma_curvy gamma_tbl = {
	.gamma_len = 33,
	.bl_len = 8,
	.ref_y_gamma = {0, 0, 0, 1, 3, 5, 9, 13, 19, 26, 37, 48,
					62, 78, 98, 120, 145, 173, 202, 238, 278,
					322, 365, 416, 466, 515, 569, 640, 714, 789,
					864, 946, 1024},
	...
	.ref_bl_lvl = {0, 48, 112, 169, 269, 422, 727, 1024},
...
static struct regulator *l20_1v8;

static void holiday_panel_power(int on)
{
	static int init;
	int ret;
	int rc;
	...
	if (!init) {
		l19_2v85 = regulator_get(NULL, "8058_l19");
		if (IS_ERR(l19_2v85)) {
			PR_DISP_ERR("%s: unable to get 8058_l19\n", __func__);
			goto fail;
		}

		l20_1v8 = regulator_get(NULL, "8058_l20");
		if (IS_ERR(l20_1v8)) {
...
fail:
	if (l19_2v85)
		regulator_put(l19_2v85);
	if (l20_1v8)
		regulator_put(l20_1v8);
...
/* [DISPLAY]: workaround for ESD isse */
/* Temp mark it */
/* #define HOY_ESD_FIXUP */
#define ESDLOOPCOUNT   60

#ifdef HOY_ESD_FIXUP
static int holiday_esd_fixup(uint32_t data)
{
	static int check_count = 0;
	unsigned char pwrVal;

	if (++check_count == ESDLOOPCOUNT) {
		check_count = 0;
		...
		if (0x9C != pwrVal) {
			...
			if (0x9C != pwrVal) {
				PR_DISP_INFO("%s: power status of driver IC = 0x%02X\n", __func__, pwrVal);
				PR_DISP_INFO("-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n");
				PR_DISP_INFO("%s: driver IC failure deteted, could be ESD\n", __func__);
				PR_DISP_INFO("-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n");
				mipi_orise_recover_chip();
			}
		}
	}

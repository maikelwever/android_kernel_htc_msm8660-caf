...
void __init holiday_allocate_fb_region(void)
{
	unsigned long size;
...
struct mdp_table_entry holiday_color_v11[] = {
	{0x93400, 0x0222, 0x0},
	{0x93404, 0xFFE4, 0x0},
	{0x93408, 0xFFFD, 0x0},
	{0x9340C, 0xFFF1, 0x0},
...
void __init holiday_mdp_writeback(struct memtype_reserve* reserve_table)
{
...
static int holiday_lcd_on(struct platform_device *pdev)
{
  ...
  if (panel_type == PANEL_ID_SHR_SHARP_NT) {
			printk(KERN_INFO "holiday_lcd_on PANEL_ID_SHR_SHARP_NT\n");
...
static int holiday_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static void holiday_set_backlight(struct msm_fb_data_type *mfd)
{
	if (panel_type == PANEL_ID_SHR_SHARP_NT) {
		led_pwm1[1] = holiday_shrink_pwm((unsigned char)(mfd->bl_level));
	}
	else {
...
static int __devinit holiday_lcd_probe(struct platform_device *pdev)
{
...
static struct platform_driver this_driver = {
	.probe  = holiday_lcd_probe,
	.driver = {
	.name   = "mipi_novatek",
	},
};

static struct msm_fb_panel_data holiday_panel_data = {
	.on	       = holiday_lcd_on,
	.off	       = holiday_lcd_off,
	.set_backlight = holiday_set_backlight,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0, 0, 0};

static int mipi_holiday_device_register(const char* dev_name, struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	holiday_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &holiday_panel_data,
		sizeof(holiday_panel_data));
	if (ret) {
...
static int __init mipi_cmd_novatek_blue_qhd_pt_init(void)
{
	...
	ret = mipi_holiday_device_register("mipi_novatek", &pinfo, MIPI_DSI_PRIM,
...
static int __init holiday_panel_init(void)
{
...
device_initcall_sync(holiday_panel_init);

/* 3D Panel */
static void holiday_3Dpanel_on(bool bLandscape)
{
...
//	led_brightness_value_set("lcd-backlight", 255);
	...
	if (system_rev >= 1) {
		...
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HOLIDAY_3DLCM_PD), &pwm_gpio_config);
		if (rc < 0)
			pr_err("%s pmic gpio config gpio %d failed\n", __func__, PM8058_GPIO_PM_TO_SYS(HOLIDAY_3DLCM_PD));
	}

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HOLIDAY_3DCLK), &clk_gpio_config_on);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, HOLIDAY_3DCLK);
	...
	if (bLandscape) {
		gpio_set_value(HOLIDAY_CTL_3D_1, 1);
		gpio_set_value(HOLIDAY_CTL_3D_2, 1);
		gpio_set_value(HOLIDAY_CTL_3D_3, 1);
		gpio_set_value(HOLIDAY_CTL_3D_4, 0);
	} else {
		gpio_set_value(HOLIDAY_CTL_3D_1, 1);
		gpio_set_value(HOLIDAY_CTL_3D_2, 1);
		gpio_set_value(HOLIDAY_CTL_3D_3, 0);
		gpio_set_value(HOLIDAY_CTL_3D_4, 1);
	}
}

static void holiday_3Dpanel_off(void)
{
	int rc;
	if (system_rev >= 1) {
		...
		rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HOLIDAY_3DLCM_PD), &pwm_gpio_config);
		if (rc < 0)
			pr_err("%s pmic gpio config gpio %d failed\n", __func__, HOLIDAY_3DLCM_PD);
	}
	pwm_disable(pwm_3d);

	rc = pm8xxx_gpio_config(PM8058_GPIO_PM_TO_SYS(HOLIDAY_3DCLK), &clk_gpio_config_off);
	if (rc < 0)
		pr_err("%s pmic gpio config gpio %d failed\n", __func__, HOLIDAY_3DCLK);
	gpio_set_value(HOLIDAY_CTL_3D_1, 0);
	gpio_set_value(HOLIDAY_CTL_3D_2, 0);
	gpio_set_value(HOLIDAY_CTL_3D_3, 0);
	gpio_set_value(HOLIDAY_CTL_3D_4, 0);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case BARRIER_LANDSCAPE:
		holiday_3Dpanel_on(true);
		break;
	case BARRIER_PORTRAIT:
		holiday_3Dpanel_on(false);
		break;
	default:
		break;
	}
...
static int holiday_3Dpanel_probe(struct platform_device *pdev)
{
	int err = 0;
	printk(KERN_INFO "%s\n", __func__);
...
static int holiday_3Dpanel_remove(struct platform_device *pdev)
{
	printk(KERN_INFO "%s\n", __func__);
	if (pwm_3d) {
		pwm_free(pwm_3d);
...
struct platform_driver holiday_3Dpanel_driver = {
	.probe	= holiday_3Dpanel_probe,
	.remove	= holiday_3Dpanel_remove,
	.driver	= {
		.name = "panel_3d",
	},
};

static int __init holiday_3Dpanel_init(void)
{
	pr_info("%s(%d)\n", __func__, __LINE__);
	return platform_driver_register(&holiday_3Dpanel_driver);
}

static void __exit holiday_3Dpanel_exit(void)
{
	platform_driver_unregister(&holiday_3Dpanel_driver);
}

module_init(holiday_3Dpanel_init);
module_exit(holiday_3Dpanel_exit);
